...
function Return() {
	...
	const returnBook = async (transactionId, borrowerId, bookId, due) => {
		try {
			...
			alert(t("bookReturnedSuccessfully"));
		} catch (err) {
			console.log(err);
		}
	};
	...
	const convertToIssue = async transactionId => {
		try {
			...
			setExecutionStatus("Completed");
			alert(t("bookIssuedSuccessfully"));
		} catch (err) {
			console.log(err);
		}
	};
	...
	return (
		<div>
			...
			<p className="dashboard-option-title">
				{t("dashboardOptionTitleIssued")}
			</p>
			...
			<p className="dashboard-option-title">
				{t("dashboardOptionTitleReserved")}
			</p>
			<table className="admindashboard-table">
				<tr>
					<th>{t("bookName")}</th>
